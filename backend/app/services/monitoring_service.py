    ...
    async def get_dashboard_stats(db: AsyncSession) -> Dict[str, Any]:
        ...
        async with httpx.AsyncClient() as client:
            try:
                ...
                avg_cpu = (
                    sum(node["cpu"] for node in processed_nodes) / len(processed_nodes)
                    if processed_nodes
                    else 0
                )
                avg_mem = (
                    sum(node["memory"] for node in processed_nodes)
                    / len(processed_nodes)
                    if processed_nodes
                    else 0
                )
                ...
                avg_disk = (
                    sum(res.disk_usage or 0 for res in all_db_resources)
                    / len(all_db_resources)
                    if all_db_resources
                    else 0
                )
