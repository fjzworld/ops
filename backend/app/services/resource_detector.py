...
class ResourceDetector:
        ...
        try:
            ...
            try:
                ...
                return {
                    ...
                    "os_version": "Unknown",
                    "kernel_version": uname_output.split()[2]
                    if len(uname_output.split()) > 2
                    else "Unknown",
                }
            except Exception:
                return {
    ...
    def get_metrics_and_status(
        ...
        try:
            ...
            if service_name:
                try:
                    status_out = self.execute_command(
            ...
            if result["status"] == "active":
                ...
                if mw_type == "mysql":
                    ...
                    if output:
                        ...
                        if uptime:
                            metrics["uptime"] = int(uptime.group(1))

                        threads = re.search(r"Threads:\s+(\d+)", output)
                        if threads:
                            metrics["threads"] = int(threads.group(1))

                        open_tables = re.search(r"Open tables:\s+(\d+)", output)
                        if open_tables:
                            metrics["open_tables"] = int(open_tables.group(1))

                        qps = re.search(r"Queries per second avg:\s+([\d\.]+)", output)
                        if qps:
                            metrics["qps"] = float(qps.group(1))
                    ...
                    for binary in cmds:
                        ...
                        try:
                            ...
                            out_str = stdout.read().decode("utf-8").strip()
                            if out_str and (
                                "redis_version" in out_str
                                or "connected_clients" in out_str
                            ):
                                output = out_str
                                break
                        except Exception:
    ...
    def detect_service_name(self, mw_type: str) -> Optional[str]:
        ...
        for svc_name in candidates:
            try:
                # 检查服务是否存在
                output = self.execute_command(
                    f"systemctl list-unit-files {svc_name}.service 2>/dev/null | grep -q {svc_name} && echo 'found'"
                )
                if output and "found" in output:
                    logger.info(
                        f"Detected service name: {svc_name} for type: {mw_type}"
                    )
                    return svc_name
            except Exception as e:
    ...
    def verify_middleware(
        ...
        try:
            ...
            if service_name:
                try:
                    service_status = self.execute_command(
                    ...
                    if service_status == "active":
                        ...
                        result["message"] = (
                            f"服务 {service_name} 未运行 (状态: {service_status})"
                        )
                        return result
                ...
                if detected_service:
                    ...
                    try:
                        service_status = self.execute_command(
                            f"systemctl is-active {detected_service} 2>/dev/null || echo 'unknown'"
                        )
                        service_status = service_status.strip()
                        if service_status == "active":
                            result["service_active"] = True
                            ...
                            result["service_active"] = True
                            result["details"]["service_status"] = (
                                f"{service_status} (auto-detected)"
                            )
                    except Exception:
                        result["service_active"] = True
                        result["details"]["service_status"] = (
                            f"check failed (auto-detected: {detected_service})"
                        )
                else:
                    # 未检测到服务名，跳过服务检查
                    result["service_active"] = True
                    result["details"]["service_status"] = (
                        "skipped (no service name detected)"
                    )
    ...
    def _verify_mysql_auth(
        self, port: int, username: str = None, password: str = None
    ) -> Dict[str, any]:
        """验证MySQL认证"""
    ...
    def detect_log_path(self, mw_type: str, service_name: str = None) -> Dict[str, any]:
        ...
        """
        result = {"found": False, "path": None, "candidates": []}
        ...
        try:
            if mw_type == "mysql":
                ...
                try:
                    ps_output = self.execute_command(
                        "ps aux | grep mysqld | grep -v grep"
                    )
                    logger.info(
                        f"[LogDetect] MySQL ps output: {ps_output[:200] if ps_output else 'empty'}"
                    )
                    if ps_output:
                        # 尝试提取 --log-error 参数
                        ...
                        if log_match:
                            ...
                            logger.info(
                                f"[LogDetect] Found --log-error param: {log_path}"
                            )
                            # 验证文件是否存在
                ...
                for path in common_paths:
                    try:
                        ...
                        logger.info(
                            f"[LogDetect] Check path {actual_path}: {'exists' if exists else 'not found'}"
                        )
                        if exists:
                            ...
                            if self.execute_command(check_cmd).strip() == "exists":
                                ...
                                if logfile_line and "logfile" in logfile_line:
                                    ...
                                    if (
                                        ...
                                        if (
                                            self.execute_command(check_cmd).strip()
                                            == "exists"
                                        ):
                                            result["found"] = True
                                            result["path"] = log_path
