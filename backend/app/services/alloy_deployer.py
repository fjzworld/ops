...
def deploy_alloy_agent(
    ...
    try:
        ...
        def execute(cmd, sudo=True):
            ...
            if exit_status != 0:
                raise RuntimeError(
                    f"Command '{cmd}' failed with exit status {exit_status}: {err}"
                )
            return out
        ...
        if not binary_path:
            raise RuntimeError(
                "Could not find alloy binary in extracted archive at /tmp/alloy_extract"
            )
        ...
        try:
            ...
            check_cmd = (
                f"curl -m 5 -s -o /dev/null -w '%{{http_code}}' {prometheus_url}"
            )
            ...
            if code_str == "000":
                ...
                log(
                    "The remote server (192.168.1.228) CANNOT reach your IP (192.168.3.26)."
                )
...
"""
        ...
        if status == "active":
            ...
            log(
                f"Deployment finished but service status is: {status} (exit code: {exit_status})"
            )
            # Get service logs
            try:
