...
async def create_middleware(
    ...
    try:
        ...
        if middleware_data.password_plain:
            db_middleware_dict["password_enc"] = encrypt_string(
                middleware_data.password_plain
            )
...
async def verify_middleware_config(
    ...
    try:
        ...
        if result.get("details", {}).get("detected_service_name"):
            result["suggested_service_name"] = result["details"][
...
async def control_middleware(
    ...
    logger.info(
        f"Executing {action} on service {middleware.service_name} for resource {middleware.resource_id}"
    )
...
async def get_middleware_metrics(
    ...
    try:
        ...
        detector = ResourceDetector(creds)
        mw_password = (
            decrypt_string(middleware.password_enc) if middleware.password_enc else None
        )
...
async def log_stream_endpoint(
    ...
    try:
        ...
        stdin, stdout, stderr = await loop.run_in_executor(
            None, lambda: detector.client.exec_command(cmd, get_pty=False)
        )
            ...
            while True:
                ...
                if not line:
                    ...
                    err = await loop.run_in_executor(
                        None, lambda: stderr.read().decode()
                    )
                    if err:
