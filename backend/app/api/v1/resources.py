...
from app.services.resource_detector import probe_server
...
async def list_resources(
    ...
    """List all resources"""
    logger.info(
        f"LIST_RESOURCES_START: user={current_user.username}, type={resource_type}, status={status}"
    )
...
async def get_resource(
    ...
    resource.has_credentials = bool(
        resource.ssh_password_enc or resource.ssh_private_key_enc
    )
    return resource
...
async def create_resource(
    ...
    result = await db.execute(
        select(Resource).filter(Resource.name == resource_data.name)
    )
    ...
    if resource_data.ip_address and (
        resource_data.ssh_password or resource_data.ssh_private_key
    ):
        auto_discovery = True
        try:
            # 1. Probe Server
            ...
            if not resource_data.hostname:
                resource_data.hostname = probe_info.hostname
            if not resource_data.cpu_cores:
                resource_data.cpu_cores = probe_info.cpu_cores
            if not resource_data.memory_gb:
                resource_data.memory_gb = probe_info.memory_gb
            if not resource_data.disk_gb:
                resource_data.disk_gb = probe_info.disk_gb
            if not resource_data.os_type:
                resource_data.os_type = f"{probe_info.os_type} {probe_info.os_version}"
    ...
    # Prepare DB object
    db_resource_dict = resource_data.dict(
        exclude={"ssh_password", "ssh_private_key", "backend_url"}
    )

    # Add encrypted credentials if available
    if resource_data.ssh_password:
        db_resource_dict["ssh_password_enc"] = encrypt_string(
            resource_data.ssh_password
        )
    if resource_data.ssh_private_key:
        db_resource_dict["ssh_private_key_enc"] = encrypt_string(
            resource_data.ssh_private_key
        )
            ...
            try:
                ...
                if resource_data.ssh_private_key:
                    ...
                    pkey = paramiko.RSAKey.from_private_key(
                        StringIO(resource_data.ssh_private_key)
                    )
    ...
    if auto_discovery:
        try:
            ...
            if success:
                ...
                logger.warning(
                    f"Agent deployment failed for {db_resource.name}. Logs: {logs}"
                )
...
async def delete_resource(
    ...
    if delete_request and delete_request.uninstall_agent and resource.ip_address:
        logger.info(
            f"准备卸载 Agent: host={resource.ip_address}, user={delete_request.ssh_username}"
        )
    ...
    response = {"message": "资源已成功删除", "agent_uninstalled": agent_uninstalled}
...
def merge_metrics(
    cpu_results, mem_results, disk_results, net_in_results, net_out_results
) -> List[dict]:
    merged_data = {}
...
async def get_metrics_history(
    ...
    return {"resource_id": resource_id, "metrics": metrics, "processes": []}
...
async def get_disk_partitions(
    ...
    for item in avail_data:
        ...
        if m in partitions:
            ...
            partitions[m]["used_gb"] = round(
                partitions[m].get("total_gb", 0) - partitions[m]["avail_gb"], 2
            )
...
async def get_resource_stats(
    ...
    inactive_query = (
        select(func.count())
        .select_from(Resource)
        .filter(Resource.status == ResourceStatus.INACTIVE)
    )
...
async def deploy_alloy_monitoring_agent(
    ...
    try:
        ...
        return {"task_id": task.id, "message": "Deployment started"}
    except Exception as e:
        logger.error(
            f"Failed to queue Alloy deployment for resource {resource_id}: {e}",
            exc_info=True,
        )
