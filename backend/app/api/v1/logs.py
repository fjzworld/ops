...
async def search_logs(
    ...
    start: Optional[str] = Query(
        None, description="Start time (ISO string or nanoseconds)"
    ),
    end: Optional[str] = Query(
        None, description="End time (ISO string or nanoseconds)"
    ),
    limit: int = Query(1000, le=5000, description="Max logs to return"),
    direction: LogDirection = Query(
    ...
    """
    params = {"query": query, "limit": limit, "direction": direction.value}
    ...
    async with httpx.AsyncClient() as client:
        try:
            ...
            if response.is_error:
                raise InternalServerError(message=f"Loki error: {response.text}")

            return response.json()

        except httpx.RequestError as exc:
            raise InternalServerError(message=f"Could not connect to Loki: {str(exc)}")
...
async def get_labels(
    ...
    async with httpx.AsyncClient() as client:
        try:
            response = await client.get(
                f"{settings.LOKI_URL}/loki/api/v1/labels", params=params, timeout=10.0
            )
...
async def deploy_promtail(
    resource_id: int,
    loki_host: Optional[str] = Query(
        None, description="Loki host IP/hostname reachable from target server"
    ),
