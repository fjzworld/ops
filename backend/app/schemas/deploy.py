...
class DeployExecuteRequest(BaseModel):
    ...
    resource_ids: List[int] = Field(..., min_length=1, description="Target resource IDs from CMDB")
    restart_keepalived: bool = Field(default=False, description="Whether to restart keepalived (for 2-server HA frontend)")
    restart_container: bool = Field(default=True, description="Whether to restart container after backend/algorithm deploy")
