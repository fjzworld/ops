...
def update_metrics(
    resource_id: str, resource_name: str, ip_address: str, metrics: dict
):
    """
    ...
    if "disk_partitions" in metrics and metrics["disk_partitions"]:
        for part in metrics["disk_partitions"]:
            ...
            mountpoint = (
                part.get("mountpoint", "unknown") if is_dict else part.mountpoint
            )
            ...
            part_labels = {**labels, "mountpoint": mountpoint, "device": device}
            try:
...
def clear_metrics(resource_id: str, resource_name: str, ip_address: str):
    ...
    try:
        ...
        logger.info(
            f"Cleared Prometheus metrics for resource {resource_id} ({resource_name})"
        )
    except Exception as e:
...
def update_resource_status(
    resource_id: str, resource_name: str, ip_address: str, is_online: bool
):
    """
    ...
    if not is_online:
        ...
        logger.info(
            f"Resource {resource_id} ({resource_name}) marked as offline, metrics cleared"
        )
    else:
