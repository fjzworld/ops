...
def test_get_ssh_credentials_decryption_fail(mock_decrypt):
    ...
    resource = Resource(ip_address="1.2.3.4", ssh_password_enc="enc_pass")
...
def test_get_ssh_credentials_no_creds():
    resource = Resource(
        ip_address="1.2.3.4", ssh_password_enc=None, ssh_private_key_enc=None
    )
...
def test_build_ssh_credentials():
    creds = CredentialService.build_ssh_credentials(
        ip_address="1.2.3.4", port=2222, username="admin", password="password"
    )
